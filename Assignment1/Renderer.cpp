...
void Renderer::nanogui_init(GLFWwindow* window)
{
	...
	gui_1->addVariable("Rotate Angle a:", m_animation-> x_angles) ->setSpinnable(true);
	gui_1->addVariable("Rotate Angle b:", m_animation-> y_angles)->setSpinnable(true);
	gui_1->addButton("Rotate Local X", []() { m_animation-> rotateLocalX(); });
	gui_1->addButton("Rotate Global Y", []() { m_animation->rotateGlobalY(); });
	gui_1->addButton("Reset model", []() { m_animation-> reset(); });
